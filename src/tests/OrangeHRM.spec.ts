...
test.describe('OrangeHRM - Login Tests', () => {
    ...
    test('TC003: Successful Admin login redirects to Dashboard', async ({ loginPage, page }) => {
    ...
    test('TC004: Login with invalid username shows error message', async ({ loginPage, page }) => {
        await loginPage.open();
    ...
    test('TC005: Login with invalid password shows error message', async ({ loginPage, page }) => {
        await loginPage.open();
    ...
    test('TC006: Login with empty credentials shows required field errors', async ({ loginPage, page }) => {
        await loginPage.open();
...
test.describe('OrangeHRM - Navigation Tests', () => {
    ...
    test('TC007: Navigate to Admin panel after login', async ({ loginPage, homePage, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
    ...
    test('TC008: Navigate back to Dashboard from Admin panel', async ({ loginPage, homePage, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
        await homePage.openAdminPanel();
        await expect(page).toHaveURL(/viewSystemUsers/);
...
test.describe('OrangeHRM - Admin Panel Tests', () => {
    ...
    test('TC009: Add new user form loads with correct fields', async ({ loginPage, homePage, adminPanel, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
        await homePage.openAdminPanel();
    ...
    test('TC010: Select ESS user role in Add User form', async ({ loginPage, homePage, adminPanel, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
        await homePage.openAdminPanel();
        await adminPanel.clickAddUser();
    ...
    test('TC011: Search for existing user in Admin panel returns results', async ({ loginPage, homePage, adminPanel, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
        await homePage.openAdminPanel();
...
test.describe('OrangeHRM - Logout Tests', () => {
    ...
    test('TC012: User can successfully logout and is redirected to login page', async ({ loginPage, homePage, page }) => {
        await loginPage.open();
        await loginPage.loginAs('Admin', 'admin123');
        await expect(page).toHaveURL(/dashboard/);
